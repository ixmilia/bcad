...
namespace BCad
{
    public class ColorMap
    {
        ...
        private RealColor[] colors;

        public static readonly ColorMap Default;
        public static readonly ColorMap AllBlack;

        static ColorMap()
        {
            var array = new RealColor[ArraySize];
            for (int i = 0; i < ArraySize; i++)
            {
                array[i] = RealColor.FromInt(defaultColors[i]);
            }

            Default = new ColorMap(array);
            AllBlack = Create(Enumerable.Repeat(0, 256).Select(i => RealColor.FromInt(i)));
        }

        private ColorMap (RealColor[] colors)
        {
            if (colors == null)
                throw new ArgumentNullException("colors");
            if (colors.Length != ArraySize)
                throw new ArgumentOutOfRangeException("colors", "Color array must contain " + ArraySize + " entities");
            this.colors = new RealColor[ArraySize];
            for (int i = 0; i < ArraySize; i++)
            {
        ...
        public RealColor this[IndexedColor i]
        {
            get
            {
        ...
        public static ColorMap Create(IEnumerable<RealColor> colors)
        {
            if (colors == null)
                throw new ArgumentNullException("colors");
            return new ColorMap(colors.ToArray());
        }

        public static ColorMap Parse(string str)
        {
            if (str == null)
                throw new ArgumentNullException("str");
            var parts = str.Split(';');
            if (parts.Length != ArraySize)
                throw new ArgumentOutOfRangeException("Expected string with " + ArraySize + " semi-colon separated values");
            var array = new RealColor[ArraySize];
            for (int i = 0; i < ArraySize; i++)
            {
                array[i] = parts[i].Trim().ParseColor();
            }

            return new ColorMap(array);
        }
        ...
        public override string ToString()
        {
            ...
            {
                rows.Add(string.Join(";", colors.Skip(row * 8).Take(8).Select(c => c.ToColorString())));
            }
