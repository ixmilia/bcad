...
namespace IxMilia.BCad.UI.Controls
{
    ...
    {
        private SvgPlotterViewModel _viewModel;

        [Import]
        public IWorkspace Workspace { get; set; }

        public SvgPlotterControl()
        {
            InitializeComponent();
        }
        ...
        {
            var fileName = await Workspace.FileSystemService.GetFileNameFromUserForWrite(new[] { new FileSpecification("SVG files", new[] { ".svg" }) });
            if (fileName != null)
            {
